# ChromeProfiles

PowerShell scripts for launching Google Chrome with a separate user data profile per Windows virtual desktop.

## What this does

- `Launch-Chrome.ps1` starts Chrome with an isolated profile directory.
- `Create-Shortcut.ps1` generates a desktop shortcut that runs the launcher.
- `chrome_desktop.ico` is the generated shortcut icon.

Each virtual desktop gets its own profile folder under `C:\ChromeProfiles\virtual_desktop_[N]\`.

## Requirements

- Windows
- PowerShell 5.1 or later
- Google Chrome installed in the default location:
  `C:\Program Files\Google\Chrome\Application\chrome.exe`

If Chrome is installed somewhere else, update the `chrome.exe` path in both scripts.

## Launching Chrome

Run the launcher directly from PowerShell:

```powershell
.\Launch-Chrome.ps1
```

Optional parameters:

```powershell
.\Launch-Chrome.ps1 -Desktop 3
.\Launch-Chrome.ps1 -Url "https://example.com"
```

### Parameters

- `-Desktop` uses a specific virtual desktop number instead of the current one.
- `-Url` opens Chrome with a specific URL.

## What the launcher does

`Launch-Chrome.ps1`:

- Detects the current virtual desktop by reading the Windows registry.
- Builds a profile name like `virtual_desktop_1`, `virtual_desktop_2`, and so on.
- Creates a separate user data folder for that desktop if it does not already exist.
- Starts Chrome with `--user-data-dir` and `--new-window`.

## Creating the desktop shortcut

Run:

```powershell
.\Create-Shortcut.ps1
```

This will:

- Extract the Chrome icon and save it as `chrome_desktop.ico`.
- Create a desktop shortcut named `Chrome (Virtual Desktop).lnk`.
- Configure the shortcut to launch the PowerShell script hidden.

## Folder layout

```text
C:\Scripts\ChromeProfiles\
  Create-Shortcut.ps1
  Launch-Chrome.ps1
  chrome_desktop.ico
```

Running the launcher creates profile data here:

```text
C:\ChromeProfiles\virtual_desktop_[N]\
```

## Notes

- The scripts are intended for Windows virtual desktops.
- The launcher defaults to the current virtual desktop when `-Desktop` is not provided.
- The shortcut generator overwrites `chrome_desktop.ico` when run.
- Each profile maintains separate bookmarks, extensions, history, and settings.
